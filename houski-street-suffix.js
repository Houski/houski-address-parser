...
/**
MODIFIED BY ALEX WILKINSON OF HOUSKI TO MAKE ADDRESS ABBREVIATION AND EXPANSION DETERMINSTIC
 */

"use strict";

const street_types = require("./houski-street-types");
...
function flatten(types, key) {
  return types
    .map((type) => {
      const abbrs = new Set(type.abbrs).add(type.suffix).add(type.standardAbbr);
